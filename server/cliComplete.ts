/**
 * Tab-completion metadata for the CLI served to `<web-console>`.
 * Top-level names come from cliHelp.ts (same as `help` output).
 */
...
export function completeCliLine(line: string): CliCompleteResult {
  ...
  if (words.length === 1) {
    ...
    // Commands with only flags (login, logout, help, …)
