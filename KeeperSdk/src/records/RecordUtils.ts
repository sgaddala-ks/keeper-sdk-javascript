...
function resolveFormatRecordOptions(
  showDetailsOrOptions?: boolean | FormatRecordOptions,
): {
  showDetails: boolean;
  unmask: boolean;
} {
...
function formatFieldValue(field: RecordField, unmask: boolean): string {
  ...
  const isSensitive =
    field.type === FieldType.Password || isTotp || field.privacyScreen === true;
...
export function formatRecordFields(
  ...
  const notes =
    record.version <= RecordVersion.Legacy ? record.data?.notes : undefined;
...
export function formatRecord(
  ...
  const { showDetails, unmask } =
    resolveFormatRecordOptions(showDetailsOrOptions);
  const summary = getRecordSummary(record);
  const lines: string[] = [
    RECORD_SEPARATOR,
  ...
  if (showDetails) {
    ...
    if (code) {
      lines.push(
        `Two Factor Code: ${code.code}    valid for ${code.secondsRemaining} sec`,
      );
    }
  }
