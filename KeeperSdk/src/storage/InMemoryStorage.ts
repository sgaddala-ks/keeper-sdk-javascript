...
export class InMemoryStorage implements VaultStorage {
  ...
  public async get<T extends VaultStorageKind>(
    kind: T,
    uid?: string,
  ): Promise<VaultStorageResult<T>> {
  ...
  public async addDependencies(dependencies: Dependencies): Promise<void> {
    for (const [parentUid, children] of Object.entries(dependencies)) {
      ...
      const seenChildUids = new Set(
        existing.map((dependency) => dependency.uid),
      );
  ...
  public getByUid<T extends VaultStorageData>(
    kind: VaultStorageKind,
    uid: string,
  ): T | undefined {
  ...
  private extractUid(item: VaultStorageData): string {
    ...
    if (item.kind === VaultObjectKind.User && accountUidStr)
