...
export function recordPermissionToText(
  canEdit: boolean,
  canShare: boolean,
): string {
...
function buildAccountUidEmailMap(
...
async function collectSharedFolderPermissions(
  ...
  for (const sharedUser of storage.getAll<DSharedFolderUser>(
    ...
    const permissionText = userPermissionToText(
      sharedUser.manageUsers,
      sharedUser.manageRecords,
    );
    let name = resolveUserDisplayName(
  ...
  for (const sharedTeam of storage.getAll<DSharedFolderTeam>(
    ...
    const permissionText = userPermissionToText(
      sharedTeam.manageUsers,
      sharedTeam.manageRecords,
    );
...
async function buildFolderSubtree(
  ...
  const userFolder = storage.getByUid<DUserFolder>(
    FolderKind.UserFolder,
    folderUid,
  );
  const sharedFolder = storage.getByUid<DSharedFolder>(
  ...
  for (const childFolder of listed.folders) {
    ...
    node.children.push(
      await buildFolderSubtree(storage, childFolder.uid, opts),
    );
  }
  ...
  if (opts.showRecords && "records" in listed) {
    ...
    node.records = records.map((recordRow) => {
      const record = storage.getByUid<DRecord>(
        VaultObjectKind.Record,
        recordRow.uid,
      );
      const title = record ? getRecordTitle(record) : recordRow.name;
      return {
        display: formatTreeRecordName(title, opts.verbose, recordRow.uid),
      };
    });
  }
...
async function buildVaultRootTree(
  storage: InMemoryStorage,
  opts: BuildOpts,
): Promise<FolderTreeNode> {
  const node: FolderTreeNode = { displayName: "", children: [] };
  ...
  for (const folderRow of rows) {
    node.children.push(
      await buildFolderSubtree(storage, folderRow.uid, optsWithPromoted),
    );
  }
  const listed = await listFolder(storage, {
    folderUid: undefined,
...
function renderNode(
  ...
  for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
    ...
    if (
      item.kind === TreeItemKind.Permission ||
      item.kind === TreeItemKind.Record
    ) {
