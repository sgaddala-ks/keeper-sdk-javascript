...
export class KeeperVault {
  ...
  private readonly config: Required<
    Omit<KeeperVaultConfig, "storage" | "sessionStorage" | "authUI">
  >;
  private readonly log: ILogger;
  private synced = false;
  private batchDepth = 0;
  private readonly folderSession: VaultFolderSession =
    FolderManager.createSession();
  ...
  constructor(config?: KeeperVaultConfig) {
    ...
    this.sessionManager =
      ...
      new SessionManager(this.config.configDir || undefined);
    this.authUI =
      config?.authUI ??
      getSdkPlatform().createAuthUI(this.config.useConsoleAuth);
    ...
    const sessionStorage: SessionStorage =
      options?.useSessionResumption === false
        ? {
            ...
            saveCloneCode: (h, u, c) =>
              this.sessionManager.saveCloneCode(h, u, c),
            getSessionParameters: () =>
              this.sessionManager.getSessionParameters(),
            saveSessionParameters: (p) =>
              this.sessionManager.saveSessionParameters(p),
          }
        : this.sessionManager;
  ...
  public async loginWithSessionToken(
    username: string,
    sessionToken: string,
  ): Promise<void> {
    const deviceConfig = await this.sessionManager.getDeviceConfig(
      this.config.host,
    );
  ...
  public async resumeSession(): Promise<void> {
    ...
    const cloneCode = await this.sessionManager.getCloneCode(
      this.config.host,
      username,
    );
    if (!cloneCode) {
  ...
  public findRecord(uidOrTitle: string): DRecord | undefined {
    ...
    return this.getRecords().find(
      (record) => getRecordTitle(record).toLowerCase() === lowerUidOrTitle,
    );
  }
  ...
  public getRecordsByType(recordType: string): DRecord[] {
    return this.getRecords().filter(
      (record) => getRecordType(record) === recordType,
    );
  }
  ...
  public getRecordMetadataByUid(uid: string): DRecordMetadata | undefined {
    return this.storage.getByUid<DRecordMetadata>(
      VaultObjectKind.Metadata,
      uid,
    );
  }
  ...
  public async addUsersToTeams(
    input: AddUsersToTeamsInput,
  ): Promise<TeamUserResult> {
    return this.userManager.addUsersToTeams(input);
  }

  public async removeUsersFromTeams(
  ...
  public async updateSharedFolderPermissions(
    ...
    const result = await this.folderManager.updateSharedFolderPermissions(
      sharedFolderUid,
      permissions,
    );
    if (result.success) await this.syncIfNeeded();
  ...
  public async updateRecord(
    ...
    const result = await updateRecordOp(
      auth,
      recordUid,
      data,
      record.revision,
  ...
  public async shareRecord(
    ...
    const record =
      this.getRecordByUid(input.recordUid) || this.findRecord(input.recordUid);
    if (!record) {
      return {
  ...
  public async removeRecordShare(
  ...
  public async getRecordShareInfo(
    recordUid: string,
