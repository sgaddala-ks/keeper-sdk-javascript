...
function teamUidFromShareTeam(team: Records.IShareTeam): string | null {
  ...
  if (team.teamUid instanceof Uint8Array)
    return webSafe64FromBytes(team.teamUid);
  if (typeof team.teamUid === "string") return team.teamUid;
  return null;
}

function addShareTeams(
  target: Map<string, ShareTeamEntry>,
  list: Records.IShareTeam[] | null | undefined,
): void {
...
export async function fetchShareObjects(
  auth: Auth,
): Promise<ShareObjectsSnapshot> {
...
export async function resolvePrimaryEnterpriseId(
  auth: Auth,
...
export async function fetchTeamMemberEmails(
  auth: Auth,
  teamUid: string,
): Promise<string[]> {
